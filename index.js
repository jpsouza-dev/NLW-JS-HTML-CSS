...
const criarItemDeAtividade = (atividade) => {
  ...
  return `
    ...
    <time class="full">
          ...
          de ${formatar.mes}
          ás ${formatar.hora}</time>
  </div>
  `;
};

const atualizarListaDeAtividaes = () => {
  let section = document.querySelector(`#section-atividade`);
  section.innerHTML = "";

  if (atividades.length === 0) {
    section.innerHTML = `<p>Nehuma atividade cadastrada.</p>`;
    return;
  }
...
criarDiasSelecao = () => {
  const dias = ["2024-02-28", "2024-02-24", "2024-02-22", "2024-02-21"];
  let diasSelecao = "";
  ...
  document.querySelector(`#dia`).innerHTML = diasSelecao;
};
